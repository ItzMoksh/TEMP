...
app.config['CELERY_BACKEND'] = "db+mysql://root:pass@localhost/celery"
...
def Display2():
    ...
    if request.method == "POST":
        ...
        if "all orders of " in query.lower():
            ...
            par = [StartDate,EndDate]
            Res = ExecuteSql.delay(sql,par)
            ...
            if m is not None:
                if m.group(1):
                    ...
                    Res = ExecuteSql.delay(sql)
                    return render_template("display.html", data = Res.get())
            cols = tmp[0].strip()
            cols = cols.replace(',','","')
            where = str(tmp[1].strip())
            where = where.split(" ")
...
@celery.task(name='app4.Res = ExecuteSql')
def ExecuteSql(sql,par = None):
